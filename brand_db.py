...
TABLES = [
    "brand_core", "colors", "typography", "copy_blocks",
    "voice_tone", "design_tokens", "principles", "services", "assets",
    "mission_vision", "archetype", "brand_architecture", "brand_story",
    "customer_persona",
]
...
# (element, font_family, font_size, font_weight, line_height, letter_spacing, usage)
_SEED_TYPOGRAPHY = [
...
# (block_type, title, body, sort_order)
_SEED_COPY_BLOCKS = [
    ...
    ("about", "About Us",
     ...
     "차트를 들여다보고, 뉴스를 훑고, 데이터를 정리하고, 알림을 확인하는 일. "
     "하나하나는 사소하지만, 쌓이면 하루를 통째로 잡아먹습니다. "
     "URSxClaw는 이 반복의 무게를 기술로 덜어드립니다.\n"
     "투자 시장에서는 감정 없이 원칙대로 움직이는 트레이딩 봇이, "
     "일상에서는 정보를 수집하고 정리하고 알려주는 자동화 시스템이 여러분 대신 일합니다. "
     "24시간, 쉬지 않고, 흔들리지 않고.\n"
...
# (name, icon, headline, description, sort_order)
_SEED_SERVICES = [
    ("자동 투자", "📈", "잠든 사이에도, 일하는 동안에도.",
     "검증된 전략 기반의 트레이딩 봇이 시장을 모니터링하고, 정해진 규칙에 따라 매매합니다. 잠든 사이에도, 일하는 동안에도.", 1),
    ("일상 자동화", "⚙️", "반복 작업을 시스템이 대신합니다.",
     "뉴스 수집, 데이터 정리, 리포트 생성, 알림 전송. 매일 손으로 하던 반복 작업을 시스템이 대신합니다.", 2),
    ("한눈에 보는 대시보드", "📊", "지금 필요한 정보만.",
     "내 자산과 자동화 현황을 깔끔한 화면 하나로 확인합니다. 복잡한 지표 대신, 지금 필요한 정보만.", 3),
]
...
# (attribute, description, do_example, dont_example)
_SEED_VOICE_TONE = [
    ("담백함", "과장하지 않는다. 있는 그대로, 필요한 만큼만 말한다.",
     "검증된 전략 기반으로 매매합니다.", "혁신적인 AI가 당신의 투자를 완전히 바꿔놓습니다!"),
    ("세련됨", "군더더기 없이 깔끔하다. 문장은 짧고 리듬감이 있다.",
     "24시간, 쉬지 않고, 흔들리지 않고.", "저희 시스템은 24시간 365일 끊김 없이 안정적으로 가동됩니다."),
    ("존중", "사용자를 가르치지 않는다. '쉬운', '간단한' 같은 평가 표현을 쓰지 않는다.",
     "결과만 확인하세요. 나머지는 저희가 알아서 합니다.", "초등학생도 이해할 수 있는 쉬운 인터페이스!"),
    ("신뢰감", "기술적 근거를 보여주되, 용어를 나열하지 않는다.",
     "정해진 규칙에 따라 매매합니다.", "최첨단 알고리즘과 머신러닝 기술을 활용하여..."),
    ("직접적", "비유, 은유 없이 실제 기능과 결과로 말한다.",
     "뉴스 수집, 데이터 정리, 리포트 생성, 알림 전송.", "마치 당신의 개인 비서처럼, 든든한 조력자가..."),
]
...
# key → (title, body, sort_order)
_SEED_BRAND_STORY = {
    "origin": (
        "시작",
        "매일 같은 일을 반복하면서, 정작 중요한 일에 쓸 시간이 없었습니다.\n"
        "차트를 들여다보고, 뉴스를 훑고, 데이터를 정리하고, 알림을 확인하는 일. "
        "하나하나는 사소하지만 쌓이면 하루를 통째로 잡아먹었습니다.\n"
        "그래서 직접 만들기 시작했습니다. 반복을 맡길 수 있는 시스템을.", 1),
    "turning_point": (
        "전환점",
        "처음에는 나 하나 편하자고 만든 봇이었습니다.\n"
        "그런데 봇이 24시간 일하는 동안, 저는 처음으로 차트 앞에서 벗어나 다른 일을 할 수 있었습니다.\n"
        "이건 나만의 문제가 아니라는 걸 깨달았습니다.", 2),
    "now": (
        "지금",
        "투자 시장에서는 감정 없이 원칙대로 움직이는 트레이딩 봇이, "
        "일상에서는 정보를 수집하고 정리하고 알려주는 자동화 시스템이 일합니다.\n"
        "24시간, 쉬지 않고, 흔들리지 않고.\n"
        "복잡한 기술을 이해할 필요는 없습니다. 결과만 확인하세요.", 3),
    "manifesto": (
        "선언",
        "우리는 기술을 자랑하지 않습니다.\n"
        "우리는 결과를 보여줍니다.\n"
        "당신의 시간은 차트 앞에 묶여 있을 만큼 가볍지 않습니다.\n"
        "URSxClaw가 일하는 동안, 당신은 당신의 하루를 사세요.", 4),
}

# (name, age_range, occupation, pain_point, desire, behavior, how_we_help, sort_order)
_SEED_CUSTOMER_PERSONA = [
...
def _create_schema(c: sqlite3.Cursor):
    c.executescript(_SCHEMA_SQL)


def _seed_data(c: sqlite3.Cursor):
    now_sql = "datetime('now')"

    for key, (value, category) in _SEED_BRAND_CORE.items():
        c.execute(
            f"INSERT OR REPLACE INTO brand_core (key, value, category, updated_at) VALUES (?, ?, ?, {now_sql})",
            (key, value, category),
        )

    for name, hex_, rgb, usage, theme, role in _SEED_COLORS:
        c.execute(
            "INSERT OR IGNORE INTO colors (name, hex, rgb, usage, theme, role) VALUES (?, ?, ?, ?, ?, ?)",
            (name, hex_, rgb, usage, theme, role),
        )

    for el, family, size, weight, lh, ls, usage in _SEED_TYPOGRAPHY:
        c.execute(
    ...
    for block_type, title, body, order in _SEED_COPY_BLOCKS:
        c.execute(
            "INSERT OR IGNORE INTO copy_blocks (block_type, title, body, sort_order) VALUES (?, ?, ?, ?)",
            (block_type, title, body, order),
        )

    for name, icon, headline, desc, order in _SEED_SERVICES:
        c.execute(
            "INSERT OR IGNORE INTO services (name, icon, headline, description, sort_order) VALUES (?, ?, ?, ?, ?)",
            (name, icon, headline, desc, order),
        )

    for title, desc, order in _SEED_PRINCIPLES:
        c.execute(
            "INSERT OR IGNORE INTO principles (title, description, sort_order) VALUES (?, ?, ?)",
            (title, desc, order),
        )

    for attr, desc, do_ex, dont_ex in _SEED_VOICE_TONE:
        c.execute(
            "INSERT OR IGNORE INTO voice_tone (attribute, description, do_example, dont_example) VALUES (?, ?, ?, ?)",
            (attr, desc, do_ex, dont_ex),
        )

    for key, (value, category, desc) in _SEED_DESIGN_TOKENS.items():
        c.execute(
            f"INSERT OR REPLACE INTO design_tokens (key, value, category, description, updated_at) VALUES (?, ?, ?, ?, {now_sql})",
            (key, value, category, desc),
        )

    for key, value in _SEED_MISSION_VISION.items():
        c.execute(
            f"INSERT OR REPLACE INTO mission_vision (key, value, updated_at) VALUES (?, ?, {now_sql})",
            (key, value),
        )

    for key, value in _SEED_ARCHETYPE.items():
        c.execute(
            f"INSERT OR REPLACE INTO archetype (key, value, updated_at) VALUES (?, ?, {now_sql})",
            (key, value),
        )

    for level, name, icon, tagline, desc, target, status, order in _SEED_BRAND_ARCHITECTURE:
        c.execute(
    ...
    for key, (title, body, order) in _SEED_BRAND_STORY.items():
        c.execute(
            f"INSERT OR REPLACE INTO brand_story (key, title, body, sort_order, updated_at) VALUES (?, ?, ?, ?, {now_sql})",
            (key, title, body, order),
        )

    for name, age, occ, pain, desire, behavior, help_, order in _SEED_CUSTOMER_PERSONA:
        c.execute(
...
def query(table: str, where: str = None, params: tuple = (), limit: int = None):
    """범용 조회 — table은 화이트리스트 검증"""
